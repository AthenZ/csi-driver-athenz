...
// validateCSRExtensions validates the given certificate signing request
// contains only valid extensions, including URI sans, key usages, and extended
// key usages. Any other extensions will error.
func validateCSRExtensions(csr *x509.CertificateRequest) error {
	var el []error
...
func validateSubjectAltNameExtension(ext pkix.Extension) error {
	...
	for bytes := sequence.Bytes; len(bytes) > 0; {
		...
		if rawValue.Tag != asn1TagURI && rawValue.Tag != asn1TagDNS && rawValue.Tag != asn1TagIP {
