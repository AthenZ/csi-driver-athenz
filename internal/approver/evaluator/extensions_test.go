...
func Test_validateCSRExtentions(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"if multiple URI names exist with allowed usages, shouldn't error": {
			uris: []string{"spiffe://foo.bar", "athenz://bar.foo"},
			usages: []cmapi.KeyUsage{
				cmapi.UsageDigitalSignature,
				cmapi.UsageKeyEncipherment,
				cmapi.UsageClientAuth,
		...
		"if multiple URI names exist, ips, and allowed usages, shouldn't error": {
			uris: []string{"spiffe://foo.bar", "athenz://bar.foo"},
			ips:  []string{"1.2.3.4"},
			usages: []cmapi.KeyUsage{
				cmapi.UsageDigitalSignature,
		...
		"if multiple URI names exist, dns, ips, and allowed usages, shouldn't error": {
			uris: []string{"spiffe://foo.bar", "spiffe://bar.foo"},
			dns:  []string{"foo.bar"},
			ips:  []string{"1.2.3.4"},
			usages: []cmapi.KeyUsage{
