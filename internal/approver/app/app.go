...
import (
	...
	"github.com/AthenZ/csi-driver-athenz/internal/approver/app/options"
...
const (
	helpOutput = "A cert-manager Approver that is paired with a cert-manager Athenz CSI driver"
)
...
// NewCommand returns an new command instance of the approver component of csi-driver-athenz.
func NewCommand(ctx context.Context) *cobra.Command {
	opts := options.New()

	cmd := &cobra.Command{
		Use:   "csi-driver-athenz-approver",
		Short: helpOutput,
		Long:  helpOutput,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			evaluator := evaluator.New(evaluator.Options{
				...
				MultiTenancy:               opts.CertManager.MultiTenancy,
			})
			...
			if err := controller.AddApprover(ctx, opts.Logr, controller.Options{
				...
				return fmt.Errorf("failed to register Athenz approver controller: %w", err)
			}

			log.Info("starting Athenz approver...")
