...
var _ = framework.CasesDescribe("Approval", func() {
	...
	JustBeforeEach(func() {
		...
		serviceAccount = corev1.ServiceAccount{
			ObjectMeta: metav1.ObjectMeta{Namespace: f.Namespace.Name, GenerateName: "csi-driver-athenz-approval-test-"},
		}
		Expect(f.Client().Create(f.Context(), &serviceAccount)).NotTo(HaveOccurred())

		role = rbacv1.Role{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: "csi-driver-athenz-approval-test-",
	...
	It("should approve a valid request", func() {
		...
		certificateRequest := cmapi.CertificateRequest{
			...
			Spec: cmapi.CertificateRequestSpec{
				Request:   genCSRPEM(pk, cmapi.ECDSAKeyAlgorithm, fmt.Sprintf("spiffe://athenz.io/ns/%s/sa/%s", f.Namespace.Name, serviceAccount.Name)),
	...
	It("should deny a request with the wrong SPIFFE ID", func() {
		...
		certificateRequest := cmapi.CertificateRequest{
			...
			Spec: cmapi.CertificateRequestSpec{
				Request:   genCSRPEM(pk, cmapi.ECDSAKeyAlgorithm, fmt.Sprintf("spiffe://athenz.io/ns/%s/sa/%s", f.Namespace.Name, "not-the-right-sa")),
				Duration:  &metav1.Duration{Duration: time.Hour},
				IssuerRef: f.Config().IssuerRef,
				IsCA:      false,
				Usages:    []cmapi.KeyUsage{cmapi.UsageDigitalSignature, cmapi.UsageKeyEncipherment, cmapi.UsageClientAuth, cmapi.UsageServerAuth},
	...
	It("should deny a request with the wrong key type", func() {
		...
		certificateRequest := cmapi.CertificateRequest{
			...
			Spec: cmapi.CertificateRequestSpec{
				Request:   genCSRPEM(pk, cmapi.RSAKeyAlgorithm, fmt.Sprintf("spiffe://athenz.io/ns/%s/sa/%s", f.Namespace.Name, serviceAccount.Name)),
				Duration:  &metav1.Duration{Duration: time.Hour},
				IssuerRef: f.Config().IssuerRef,
				IsCA:      false,
				Usages:    []cmapi.KeyUsage{cmapi.UsageDigitalSignature, cmapi.UsageKeyEncipherment, cmapi.UsageClientAuth, cmapi.UsageServerAuth, cmapi.UsageCertSign},
